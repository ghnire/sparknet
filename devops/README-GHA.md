...
2. Go to your repository at [ghnire/sparknet](https://github.com/ghnire/sparknet).
3. Click on `Settings` at the top-right corner.
4. In the left sidebar, click on `Environments`.
5. Press `New environment`.
6. Name the environment `sparknet.spark.nl` and hit `Configure environment`.
...
|-------------|--------------|-------------|
| DEPLOY_HOST | sparknet.spark.nl | The hostname or IP address of your Docker Swarm manager. |
...
- [Backend Tests Workflow](https://github.com/ghnire/sparknet/actions/workflows/backend.yml)
- [Frontend Tests Workflow](https://github.com/ghnire/sparknet/actions/workflows/frontend.yml)

Upon successful completion of the tests, Docker images for the Backend (`ghcr.io/ghnire/sparknet-backend`) and Frontend (`ghcr.io/ghnire/sparknet-frontend`) will be available.

### Initiating Manual Deployment

1. Navigate to [Manual Deployment of sparknet.spark.nl](https://github.com/ghnire/sparknet/actions/workflows/manual-deploy.yml).
...
- Initiate a new deployment using the `devops/stacks/sparknet.spark.nl.yml` stack.
